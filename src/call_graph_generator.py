...
from src.ast_generator import antlr_listener
from src.contract_extractor import extract_function_from_solidity
...
def antlr_call_graph(sol_file, function_label):
    ...
    """
    listener, _, _ = antlr_listener(sol_file)
...
def generate_call_graph(path, solc_version):
    ...
    # Create a copy of the current environment and update PATH
    env = os.environ.copy()
    env["PATH"] = os.path.expanduser("~/Library/Python/3.11/bin") + ":" + env.get("PATH", "")
    
    if solc_version:
        cmd = 'solc-select use ' + solc_version.replace('^', '') + ' --always-install'
        proc = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                              shell=True, env=env)
        output = proc.stdout.decode() + proc.stderr.decode()
        if proc.returncode != 0:
            # Log the error output and continue instead of raising an exception.
            logging.warning("solc-select command returned non-zero exit code:\n" + output)
            # Optionally, you can check if the output indicates success.
            # If it does, you can ignore the error.
            # For example, if "Switched global version" appears in the output, assume it's okay.
            if "Switched global version" not in output:
                # If you want to force a fallback or exit, you could do that here.
                # For now, we'll just log and continue.
                logging.warning("Continuing despite solc-select error.")
    
    # Run Slither with the updated environment
    p = subprocess.run(['slither', path, '--print', 'call-graph'],
